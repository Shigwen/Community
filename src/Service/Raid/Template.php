...
class Template
{
	...
	public function createTemplate(User $user, Raid $newRaidTemplate, RaidCharacter $newRaidCharacter) {
		...
		$newRaidTemplate = $this->addCharacterAndServerToRaid($newRaidTemplate, $newRaidCharacter, $request->request->get('raid'));
	 ...
	 */
	public function createRaid(Raid $newRaidTemplate, RaidCharacter $newRaidCharacter) {
        $request = $this->requestStack->getCurrentRequest();

		$newRaidTemplate
		->setTemplateName(null)
		->setIdentifier($newRaidTemplate->getIsPrivate() ? $this->identifier->generate(Raid::IDENTIFIER_SIZE) : null);
	...
	public function editChosenTemplate(User $user, Raid $raidTemplateInUse, Raid $raidForm) {
		...
		$raidTemplateInUse = $this->addCharacterAndServerToRaid($raidTemplateInUse, $raidCharacter, $request->request->get('raid'));
		$raidTemplateInUse
